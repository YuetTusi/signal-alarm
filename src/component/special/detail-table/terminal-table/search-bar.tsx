...
import { Col, Row, Form, Button, DatePicker, TreeSelect, Input } from 'antd';
...
const SearchBar: FC<SearchBarProp> = ({ formRef, onSearch, onExport }) => {
    ...
    const onSubmitClick = (event: MouseEvent) => {
        ...
        onSearch(beginTime, endTime, getTypes(type), mac, deviceId);
    };
    ...
    const onExportClick = (event: MouseEvent) => {
        ...
        const { beginTime, endTime, type, mac, site } = formRef.getFieldsValue();
        const deviceId = helper.getDeviceIdFromDropdown(site);
        onExport(beginTime, endTime, getTypes(type), mac, deviceId);
    };
